...
Описание: консольная программа, которая будет по заданным данным оптимально строить 
маршрут для складского погрузчика
...
Описание команд:
make-warehouse создаёт помещение склада из n точек, всё что внутри точек считается зоной 
где можно размещать полки и находиться погрузчику
...
add-shelf создаёт полку для товаров, shelf-floors - количество этажей полки. 
Для упрощения будем считать полку всегда прямоугольной, её координаты задаются координатами 
противоположных вершин прямоугольника. 
Все товары склада будут храниться в хэш таблице.
...
add-edge создаёт именованное ребро перемещения заданного количества предмета из точки, где он лежит, 
в точку с координатами x, y
delete-edge удаляет ребро по имени

make-route создаёт оптимальный маршрут от текущих координат робота по указанным именованным рёбрам и 
записывает результат в файл filename. 
После успешного построения маршрута количество предметов на полке уменьшается.
